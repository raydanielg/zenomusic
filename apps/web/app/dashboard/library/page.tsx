...
export default function LibraryPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(data) ? data : data.history || data.songs || data.tracks || data.items || data.data || data.results || []
      setSongs(list)
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex min-w-0 flex-1 items-center gap-3">
                            <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-md bg-gradient-to-br from-orange-500 via-red-500 to-purple-600">
                              {cover && (
                                <img src={cover} alt={getSongTitle(song)} className="h-full w-full object-cover" />
                              )}
                              {!cover && (
                                <div className="flex h-full w-full items-center justify-center">
                                  <IconMusic className="h-4 w-4 text-white/50" />
                                </div>
                              )}
                            </div>
          ...
          {currentSong && (
            <div className="sticky bottom-0 z-50 border-t border-border/60 bg-background/95 backdrop-blur-lg">
              ...
              <div className="flex items-center gap-4 px-4 py-3">
                {/* Current song info */}
                <div className="flex min-w-0 flex-1 items-center gap-3">
                  <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-md bg-gradient-to-br from-orange-500 via-red-500 to-purple-600">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      {currentSong.visibility || (currentSong.isPublic ? "Public" : "Private")}
                    </p>
                  </div>
                </div>
